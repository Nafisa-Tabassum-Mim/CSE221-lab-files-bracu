...
def kruskals(n, edges, include_edge=None, exclude_edge=None):
    parent, rank = set(n)
    total_cost = 0
    edges_used = []
...
def find_second_mst(n, edges):
    ...
    mst_cost, mst_edges = kruskals(n, edges_sorted)
    if mst_cost is None:
        return -1
    ...
    for excluded_edge in mst_edges:
        current_cost, current_edges = kruskals(n, edges_sorted, exclude_edge=excluded_edge)
